...
let adminCache = { themes: [], ages: [], partTypes: [], hoTypes: [], enums: [] };
        ...
        async function loadParts() { showLoading(); try {
            ...
            let html = `<div class="card"><div class="card-header"><i class="fas fa-microchip"></i> Детали LEGO <small class="text-muted">(элементы наборов, не складские изделия)</small><div class="float-end"><button class="btn btn-sm btn-success me-2" onclick="showCreatePartModal()"><i class="fas fa-plus"></i> Создать</button><button class="btn btn-sm btn-primary" onclick="loadParts()"><i class="fas fa-sync-alt"></i> Обновить</button></div></div><div class="card-body"><div class="table-responsive"><table class="table table-bordered"><thead><tr><th>ID</th><th>Название</th><th>Цвет</th><th>Размер</th><th>Вес, г</th><th>Тип детали</th><th>Действия</th></tr></thead><tbody>`;
            for (const part of parts) html += `<tr><td>${part.id}</td><td>${escapeHtml(part.name)}</td><td>${escapeHtml(part.color)}</td><td>${escapeHtml(part.size)}</td><td>${part.weight}</td><td>${escapeHtml(part.type_name || '—')}</td><td class="action-buttons"><button class="btn btn-sm btn-danger" onclick="deletePart(${part.id})"><i class="fas fa-trash"></i></button></td></tr>`;
            html += `</tbody></table></div></div></div>`;
            document.getElementById('content').innerHTML = html;
        } catch(e) { showError(e.message); } }
